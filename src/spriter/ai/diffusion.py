...
# Inpainting: fully regenerate the masked cell.
DEFAULT_INPAINT_STRENGTH = 1.0
# Number of preceding frames used as context when inpainting the next frame.
CONTEXT_FRAMES = 3
...
def load_inpaint_pipeline(
    ...
    """Load an inpainting pipeline from a local model.

    Mirrors :func:`load_pipeline` but builds an inpainting pipeline.  Single-file
    ``.safetensors`` checkpoints are routed to the SD or SDXL inpaint pipeline
    based on the detected architecture; directories use
    ``AutoPipelineForInpainting``.  Dedicated inpainting checkpoints give the
    best results, but standard checkpoints also work.

    Args:
        model_path: Path to a diffusers model directory or ``.safetensors`` file.
        disable_safety_checker: When ``True`` (default) the NSFW safety checker
            is not loaded.

    Returns:
        A ready-to-run inpainting pipeline on the best available device.
...
def _build_inpaint_row(
    ...
    *,
    ...
    """Build the square inpaint image and mask for next-frame prediction.

    Lays out up to :data:`CONTEXT_FRAMES` previous frames followed by a noise
    cell in a single horizontal row ``[f-2][f-1][f0][NEW]``, left-padding with
    blank cells when fewer context frames are available.  The row is padded to a
    square and upscaled to *native*; the mask marks only the final (NEW) cell.

    Args:
        context_frames: Up to 3 canvas-sized RGBA arrays, oldest→newest.
        canvas_width: Sprite canvas width in pixels.
        canvas_height: Sprite canvas height in pixels.
        native: Target square side (e.g. 512 or 1024).
        seed: Optional RNG seed for the noise cell (deterministic tests).

    Returns:
        ``(image, mask, cell_bbox)`` — an RGB ``PIL.Image`` sized
        ``(native, native)``, an ``L`` mask (white over the NEW cell), and the
        ``(x0, y0, w, h)`` of the NEW cell within the native image.
    """
    cw, ch = canvas_width, canvas_height
    row_w = (CONTEXT_FRAMES + 1) * cw
    row_h = ch

    row = np.full((row_h, row_w, 3), 255, dtype=np.uint8)
    frames = list(context_frames)[-CONTEXT_FRAMES:]
    pad = CONTEXT_FRAMES - len(frames)
    for i, rgba in enumerate(frames):
        cell = np.asarray(_rgba_to_rgb_image(rgba), dtype=np.uint8)
        col = pad + i
        row[:, col * cw : (col + 1) * cw] = cell
    rng = np.random.default_rng(seed)
    row[:, CONTEXT_FRAMES * cw : (CONTEXT_FRAMES + 1) * cw] = rng.integers(
        0, 256, size=(ch, cw, 3), dtype=np.uint8
    )

    side = max(row_w, row_h)
    square = np.full((side, side, 3), 255, dtype=np.uint8)
    off_x = (side - row_w) // 2
    off_y = (side - row_h) // 2
    square[off_y : off_y + row_h, off_x : off_x + row_w] = row
    new_x0 = off_x + CONTEXT_FRAMES * cw
    new_y0 = off_y
    ...
    sx0, sy0, sw, sh = scaled(new_x0), scaled(new_y0), scaled(cw), scaled(ch)
    image = Image.fromarray(square, mode="RGB").resize(
        (native, native), Image.Resampling.LANCZOS
    )
    mask = np.zeros((native, native), dtype=np.uint8)
    mask[sy0 : sy0 + sh, sx0 : sx0 + sw] = 255
    return image, Image.fromarray(mask, mode="L"), (sx0, sy0, sw, sh)
...
def generate_next_frame(
    ...
    *,
    guidance_scale: float = DEFAULT_GUIDANCE_SCALE,
    steps: int = DEFAULT_STEPS,
    strength: float = DEFAULT_INPAINT_STRENGTH,
    seed: int | None = None,
) -> np.ndarray:
    """Predict the next animation frame by inpainting a noise cell.

    The previous frames and a masked noise cell are laid out in a row, padded to
    a square, and upscaled to the model's native resolution.  The pipeline
    inpaints the noise cell; that cell is then cropped out and returned.

    Args:
        pipeline: An inpainting pipeline from :func:`load_inpaint_pipeline`.
        context_frames: Up to 3 canvas-sized RGBA arrays, oldest→newest.
        canvas_width: Sprite canvas width in pixels.
        canvas_height: Sprite canvas height in pixels.
        prompt: Optional text prompt guiding the generation.
        guidance_scale: Classifier-free guidance scale.
        steps: Number of denoising steps.
        strength: Inpaint strength (1.0 fully regenerates the masked cell).
        seed: Optional RNG seed for the noise cell.

    Returns:
        The inpainted next frame as an ``H×W×4`` ``uint8`` RGBA array (at the
        generation resolution; callers scale it back to the canvas).
    """
    native = _native_size_for(pipeline)
    image, mask, (x0, y0, w, h) = _build_inpaint_row(
        context_frames, canvas_width, canvas_height, native, seed=seed
    )
    result = pipeline(
        prompt=prompt,
        image=image,
    ...
    out_image = result.images[0].convert("RGBA")
    cropped = out_image.crop((x0, y0, x0 + w, y0 + h))
